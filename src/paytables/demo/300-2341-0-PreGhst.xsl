				...
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						function showBonusGrid(A_canvasIdStr, A_elementStr, A_round, A_bonusGame, A_prizeIndex, A_elimNum)
						{
							...
							var winNumStr 	   = '';
							var prizeColourStr     = (bgRoundEliminated[bgIndex] > A_round) || (bgRoundEliminated[bgIndex] == 0) ? bonusCashColours[0] : bonusCashColours[1];
							var boxColourStr = (bgRoundEliminated[bgIndex] > A_round) || (bgRoundEliminated[bgIndex] == 0) ? bonusBorderColours[0] : bonusBorderColours[1]; 
						...
						for (var gameIndex = 0; gameIndex < scenarioMainGame.length; gameIndex ++)
						{
							...
							for (var letterIndex = 0; letterIndex < 3; letterIndex++)
							{
								...
								if (playLetter == "Z")
								{
									r.push('<td style="text-align:center;font:48px Arial">');
									r.push(bonusSymb);
									r.push('</td>');
								}
								else
						...
						if (doBonusGame)
						{
							...
							{
								spinStr = getTranslationByName("afterSpin", translations) + ' ' + parseInt(bgSpinIndex + 1);
								bonusPlayChar = bonusGameTurns[bgSpinIndex *2];
								bonusPlayChar2 = bonusGameTurns[(bgSpinIndex *2) +1];
								spinStr2 = '';
								...
								if (isPrizeSymb(bonusPlayChar))
								{
									elimNum = getIndexOfBonusChar(bonusPlayChar); 
					...
					{
						var bonusNum = bonusChar.charCodeAt(0) - 'A'.charCodeAt(0) + 1;

						bonusNum += (bonusNum >= 7) ? 1 : 0;
					...
					{
						var prizeNum = prizeChar.charCodeAt(0) - 'a'.charCodeAt(0) + 1;

						prizeNum += (prizeNum >= 7) ? 1 : 0;
